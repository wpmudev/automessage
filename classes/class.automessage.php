...
class automessage
{
    ...
    function __construct()
    {
        ...
        add_action('init', array(
            &$this,
            'process_user_automessage'
        ));
        add_action('init', array(
            &$this,
            'process_blog_automessage'
        ));
        ...
        add_action('load-toplevel_page_automessage', array(
            &$this,
            'add_admin_header_automessage_dash'
        ));
        add_action('load-automessage_page_automessage_blogadmin', array(
            &$this,
            'add_admin_header_automessage_blogadmin'
        ));
        add_action('load-automessage_page_automessage_useradmin', array(
            &$this,
            'add_admin_header_automessage_useradmin'
        ));

        add_action('automessage_dashboard_left', array(
            &$this,
            'dashboard_news'
        ));
        ...
        add_filter('user_row_actions', array(
            &$this,
            'add_user_to_queue_action'
        ), 99, 2);
        add_filter('ms_user_row_actions', array(
            &$this,
            'add_msuser_to_queue_action'
        ), 99, 2);
        //ms_user_row_actions
        ...
        add_filter('manage_sites_action_links', array(
            &$this,
            'add_blog_to_queue_action'
        ), 99, 3);

        add_action('load-users.php', array(
            &$this,
            'process_add_user_to_queue_action'
        ));
        add_action('load-sites.php', array(
            &$this,
            'process_add_blog_to_queue_action'
        ));
    ...
    function initialise_plugin()
    {
        ...
        register_post_type('automessage', array(
            'singular_label' => __('Messages', 'automessage'),
    ...
    function output_admin_notices()
    {
        ...
        if (isset($_GET['automessagemsg'])) {
            switch ($_GET['automessagemsg']) {
                ...
                case 1:
                    echo '<div id="message" class="updated fade"><p>' . __('User added to Automessage queue.', 'automessage') . '</p></div>';
                    break;

                case 2:
                    echo '<div id="message" class="error"><p>' . __('User could not be added to Automessage queue.', 'automessage') . '</p></div>';
                    break;

                case 3:
                    echo '<div id="message" class="updated fade"><p>' . __('Blog admin added to Automessage queue.', 'automessage') . '</p></div>';
                    break;

                case 4:
                    echo '<div id="message" class="error"><p>' . __('Blog admin could not be added to Automessage queue.', 'automessage') . '</p></div>';
                    break;
            }
        }
    ...
    function setup_menu()
    {
        ...
        add_menu_page(__('Automessage', 'automessage'), __('Automessage', 'automessage'), 'edit_automessage', 'automessage', array(
            &$this,
            'handle_dash_panel'
        ));
        ...
        if (is_multisite() && function_exists('is_super_admin') && is_super_admin() && $blog_id == 1) {
            add_submenu_page('automessage', __('Edit Blog Messages', 'automessage'), __('Blog Level Messages', 'automessage'), 'edit_automessage', "automessage_blogadmin", array(
                &$this,
                'handle_blogmessageadmin_panel'
            ));
        }

        add_submenu_page('automessage', __('Edit User Messages', 'automessage'), __('User Level Messages', 'automessage'), 'edit_automessage', "automessage_useradmin", array(
            &$this,
            'handle_usermessageadmin_panel'
        ));
        ...
        switch ($action) {
            ...
            case 'addaction':
                if ($this->add_action()) {
                    wp_safe_redirect(remove_query_arg(array(
                        'action',
                        'id'
                    ), add_query_arg('msg', $this->get_notification_message(1), wp_get_original_referer())));
                } else {
                    wp_safe_redirect(remove_query_arg(array(
                        'action',
                        'id'
                    ), add_query_arg('msg', $this->get_notification_message(2), wp_get_original_referer())));
                }
                break;
            case 'pauseaction':
                ...
                wp_safe_redirect(remove_query_arg(array(
                    'action',
                    'id'
                ), add_query_arg('msg', $this->get_notification_message(3), wp_get_original_referer())));
                break;
            case 'unpauseaction':
                $id = addslashes($_GET['id']);
                ...
                if (isset($_POST['allaction_delete'])) {
                    if (isset($_POST['allschedules'])) {
                        ...
                        wp_safe_redirect(remove_query_arg(array(
                            'action',
                            'id'
                        ), add_query_arg('msg', $this->get_notification_message(5), wp_get_original_referer())));
                    }
                }
                if (isset($_POST['allaction_pause'])) {
                    if (isset($_POST['allschedules'])) {
                        ...
                        wp_safe_redirect(remove_query_arg(array(
                            'action',
                            'id'
                        ), add_query_arg('msg', $this->get_notification_message(6), wp_get_original_referer())));
                    } else {
                        wp_safe_redirect(remove_query_arg(array(
                            'action',
                            'id'
                        ), add_query_arg('msg', $this->get_notification_message(7), wp_get_original_referer())));
                    }
                }
                if (isset($_POST['allaction_unpause'])) {
                    if (isset($_POST['allschedules'])) {
                        ...
                        wp_safe_redirect(remove_query_arg(array(
                            'action',
                            'id'
                        ), add_query_arg('msg', $this->get_notification_message(8), wp_get_original_referer())));
                    } else {
                        wp_safe_redirect(remove_query_arg(array(
                            'action',
                            'id'
                        ), add_query_arg('msg', $this->get_notification_message(9), wp_get_original_referer())));
                    }
                }
                if (isset($_POST['allaction_process'])) {
                    if (isset($_POST['allschedules'])) {
                        ...
                        wp_safe_redirect(remove_query_arg(array(
                            'action',
                            'id'
                        ), add_query_arg('msg', $this->get_notification_message(10), wp_get_original_referer())));
                    } else {
                        wp_safe_redirect(remove_query_arg(array(
                            'action',
                            'id'
                        ), add_query_arg('msg', $this->get_notification_message(11), wp_get_original_referer())));
                    }
                }
            ...
            case 'processuseraction':
                ...
                wp_safe_redirect(remove_query_arg(array(
                    'action',
                    'id'
                ), add_query_arg('msg', $this->get_notification_message(14), wp_get_original_referer())));
                break;
    ...
    function get_notification_message($i)
    {
        switch ($i) {
            case 1:
                return __('Your action has been added to the schedule.', 'automessage');
                break;

            case 2:
                return __('Your action could not be added.', 'automessage');
                break;

            case 3:
                return __('The scheduled action has been paused', 'automessage');
                break;

            case 4:
                return __('The scheduled action has been unpaused', 'automessage');
                break;

            case 5:
                return __('Please select an action to delete', 'automessage');
                break;

            case 6:
                return __('The scheduled actions have been paused', 'automessage');
                break;

            case 7:
                return __('Please select an action to pause', 'automessage');
                break;

            case 8:
                return __('The scheduled actions have been unpaused', 'automessage');
                break;

            case 9:
                return __('Please select an action to unpause', 'automessage');
                break;

            case 10:
                return __('The scheduled actions have been processed', 'automessage');
                break;

            case 11:
                return __('Please select an action to process', 'automessage');
                break;

            case 12:
                return __('The scheduled action has been deleted', 'automessage');
                break;

            case 13:
                return __('The scheduled action has been updated', 'automessage');
                break;
    ...
    function show_admin_messages()
    {
        $this->add_admin_header_automessage_core();
        if (isset($_GET['msg'])) {
            echo '<div id="message" class="updated fade"><p>' . $_GET['msg'] . '</p></div>';
        ...
        if ($results) {
            ...
            foreach ($results as $result) {
                ...
                if ($lasthook != $hook) {
                    switch ($hook) {
                        case 'wpmu_new_blog':
                            $title = __('Create new blog', 'automessage');
                            break;

                        case 'wpmu_new_user':
                            $title = __('Create new user', 'automessage');
                            break;
                    }
...
?>
    ...
    function handle_blogmessageadmin_panel()
    {
        ...
        echo '<a class="add-new-h2" href="' . remove_query_arg('msg', add_query_arg(array(
            'action' => 'newaction'
        ))) . '">' . __('Add New', 'automessage') . '</a>';
        echo "</h2>";
    ...
    function process_unsubscribe_action()
    {
        ...
        if (isset($wp_query->query_vars['namespace']) && $wp_query->query_vars['namespace'] == 'automessage') {
            ...
            if (isset($wp_query->query_vars['unsubscribe'])) {
                $unsub = $wp_query->query_vars['unsubscribe'];
            }
